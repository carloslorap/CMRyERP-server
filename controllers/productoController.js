  ...
  if (req.user) {
    ...
    try {
      ...
      if (productos.length >= 1) {
        res.status(200).send({
          data: undefined,
          message: "Ya existe un producto con ese titulo",
        });
      } else {
...
const eliminar_variedad_producto_admin = async function (req, res) {
  if (req.user) {
    ...
    if (variedad.stock == 0) {
      ...
      res
        .status(200)
        .send({ data: undefined, message: "No puede eliminar la variedad" });
    }
  } else {
    res.status(403).send({
      data: undefined,
...
const listar_producto_titulo_admin = async function (req, res) {
  if (req.user) {
...
const registrar_inventario_admin = async function (req, res) {
  if (req.user) {
    ...
    let margen_ganancia = data.ganacia_producto; //es el 30
    let monto_ganancia = Math.round(
      (data.costo_unidad * margen_ganancia) / 100
    ); //(100*30) = 3000/100 = 30
...
const obtener_inventario_admin = async function (req, res) {
  if (req.user) {
    let variedades = await Variedad.find({ stock: { $gte: 1 } })
      .select("_id sku titulo stock")
      .populate({
        path: "producto",
...
const obtener_inventario_entrada_admin = async function(req, res) {
  try {
    ...
    // Utilizamos await directamente en la consulta para obtener el resultado
    let inventario = await Inventario.find()
    ...
    // Filtramos el inventario según el año y mes
    inventario = inventario.filter((element) => {
    ...
    // Puedes enviar el inventario filtrado como respuesta al cliente
    res.status(200).send({ data: inventario });
  } catch (error) {
    // Manejamos cualquier error y devolvemos un código de estado 500
    console.error(error);
