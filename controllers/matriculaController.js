...
const send_voice = async function(req,res){
  if (req.user) {
    let id= req.params['id']
    enviar_orden_matricula(id)

    generar_actividad_matricula(id,"Se reenvio la order al cliente")
...
const disminuir_aforo_admin = async function (id) {
  ...
  await Ciclo_salon.findByIdAndUpdate(
    { _id: id },
    {
...
const enviar_orden_matricula = async function (id) {
  ...
  //OBTENER MATRICULA
  let matricula = await Matricula.findById({ _id: id })
    .populate("cliente")
    .populate("asesor");
  let detalles = await Matricula_detalle.find({ matricula: id })
    .populate("ciclo_curso")
    .populate("ciclo_salon")
    .populate("curso");
  let createdAt = moment(matricula.createdAt).format("DD/MM/YYYY");

  readHTMLFile(process.cwd() + "/mails/invoice_matricula.html", (err, html) => {
...
const obtener_comentarios_matricula_admin = async function(req,res){
  if (req.user) {
    ...
    res.status(403).send({data:undefined})
  }
}

module.exports = { generar_matricula_admin, obtener_matriculas_hoy ,obtener_matriculas_fechas,send_voice,obtener_matricula_admin,cancelar_matricula_admin,obtener_comentarios_matricula_admin};
