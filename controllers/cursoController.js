...
const Ciclo_curso = require("../models/Ciclo_curso.js");
const Ciclo_docente = require("../models/Ciclo_docente.js");
const Ciclo_salon = require("../models/Ciclo_salon.js");
const fs = require("fs");
  ...
  if (req.user) {
    ...
    try {
      ...
      if (cursos.length >= 1) {
        res.status(200).send({
          data: undefined,
          message: "Ya existe un curso con ese titulo",
        });
      } else {
      ...
      res.status(403).send({
        data: undefined,
        message: "Ocurrio un problema al registrar el curso",
      });
    }
  } else {
        ...
        if (cursos[0]._id === id) {
          if (req.files) {
            ...
            data.slug = data.titulo
              ...
              .replace(/[^\w-]+/g, "");
            data.banner = name;
            let reg = await Curso.findByIdAndUpdate(
              { _id: id },
              {
                titulo: data.titulo,
                slug: data.slug,
...
const crear_ciclo_admin = async function (req, res) {
  if (req.user) {
    ...
    console.log(format_inicio.getFullYear());
...
const listar_ciclos_admin = async function (req, res) {
  if (req.user) {
    ...
    var ciclos = await Ciclo_curso.find({ curso: id, year: year }).populate(
      "curso"
    );
    var arr = [];
    ...
    for (var item of ciclos) {
      let format_inicio =
        Date.parse(new Date(item.v_inicio + "T00:00:00")) / 1000;
...
const listar_ciclos_vencidos_admin = async function (req, res) {
  if (req.user) {
    ...
    for (var item of ciclos) {
      let format_inicio =
        Date.parse(new Date(item.f_inicio + "T00:00:00")) / 1000;
      let format_fin = Date.parse(new Date(item.f_fin + "T00:00:00")) / 1000;

      if (format_today >= format_fin) {
        let salones = await Ciclo_salon.find({ ciclo_curso: item._id });
        arr.push({ ciclo: item, salones: salones });
      }
    }
...
// (ojo carlos esta funcion solo va para cuando quieras actualizar un ciclo .....no de hecho ahora pensandolo si podrias hacerlo tmb jejeje gaaaa)
...
const listar_docentes_salon_admin = async function (req, res) {
  if (req.user) {
    let id = req.params["id"];
    let docentes = await Ciclo_docente.find({ ciclo_curso: id })
      .populate("colaborador")
      .populate("ciclo_salon");
